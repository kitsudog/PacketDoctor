...
public final class DisposableGC
{
    ...
    /** The cleanup timeout. */
    private final AtomicLong cleanupTimeout = new AtomicLong(DisposableGC.DEFAULT_CLEANUP_THREAD_TIMEOUT);
    ...
    final LinkSequence<DisposableReference> g0 = new LinkSequence<DisposableReference>("g0");

    /** The g10. */
    final LinkSequence<DisposableReference> g10 = new LinkSequence<DisposableReference>("g10");

    /** The g60. */
    final LinkSequence<DisposableReference> g60 = new LinkSequence<DisposableReference>("g60");
    ...
    private final Semaphore memorySemaphore = new Semaphore(DisposableGC.MIN_MEMORY_RELEASE);
     ...
     */
    public void drainRefQueue(long timeout) throws IllegalArgumentException, InterruptedException
    {
    ...
    void drainRefQueueLoop() throws InterruptedException
    {
        ...
        while (true)
        {
            ...
            final DisposableReference ref = (DisposableReference) refQueue.remove(timeout);

            if (ref != null)
            { // We have a reference to dispose of
                if (deltaCount == 0)
                { // First one
        ...
        {
            System.out.printf("DisposableGC: disposed of %d entries [total=%dM]%n", deltaCount, totalDisposed / 1000000);
            deltaCount = 0;
        }
    }
    ...
    public synchronized void invokeSystemGCAndWait()
    {
        ...
        try
        {
            if (isCleanupThreadActive())
            {
                ...
                memorySemaphore.tryAcquire(MIN_MEMORY_RELEASE, OUT_OF_MEMORY_TIMEOUT, TimeUnit.MILLISECONDS);
            }
            else
            {
                invokeSystemGC();
        ...
        if (vverbose)
        {
            System.out.printf("DisposableGC: waiting for System.gc to finish:" + " %dms, freed=%dMbytes%n", (System.currentTimeMillis() - ts),
    ...
    public synchronized void invokeSystemGCWithMarker()
    {
        ...
        if (markerReference != null && markerReference.get() != null || !isSystemGCReady())
        {
            return;
        }
    ...
    private final boolean isSystemGCReady()
    {
        ...
        return cleanupThreadProcessing.get() == false && (System.currentTimeMillis() - lastSystemGCInvoke) > MIN_SYSTEM_GC_INVOKE_TIMEOUT;
    }
    ...
    {
        System.out.printf("DisposableGC: current native memory allocation limits are max=%s, soft=%s%n", fb(JMemory.maxDirectMemory()),
    ...
    private void logMarker()
    {
         ...
         */
        System.out.printf("DisposableGC: soft limit breached, " + "issued a marker at %s.%d, minimum delay=%dms%n", new Time(ts), fs,
    ...
    private void logSystemGC()
    {
        ...
        System.out.printf("DisposableGC: issued JVM GC request %s.%d waited=%dms (reserved=%s, available=%s)%n", new Time(ts), fs, waited,
                fb(JMemory.reservedDirectMemory()), fb(JMemory.availableDirectMemory()));
    ...
    {
        System.out.printf("DisposableGC: [immediate=%3s(%4s)]=%3s(%7s) " + "[0sec=%3s(%6s),10sec=%3s(%6s),60sec=%3s(%6s)]=%6s%n", f(deltaCount),
                fb(deltaSize, 0), f(totalDisposed), fb(totalSize), f(g0.size()), fb(mem(g0)), f(g10.size()), fb(mem(g10)), f(g60.size()), fb(mem(g60)),
     ...
     * @throws InterruptedException the interrupted exception
     */
    public boolean waitForForcableCleanup(long timeout) throws InterruptedException
    {
