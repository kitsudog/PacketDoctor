...
public class HttpHandler extends AbstractPacketHandler {
	...
	public static Pattern REQUEST_COMMAND = Pattern
			.compile("(GET|POST) ([^ ]+) HTTP/[0-9.]+");
	public static Pattern RESPONSE_COMMAND = Pattern
			.compile("HTTP/[0-9.]+ (\\d+)( \\w+)?");
	...
	private void doHeader() {
		...
		state = new String(Arrays.copyOfRange(data, 0, 4)).equals("HTTP") ? response
				: request;
		...
		String header = new String(Arrays.copyOfRange(data, 0,
				state.headerLength));
		...
		if (headers.length > 1) {
			for (String line : Arrays.copyOfRange(headers, 1, headers.length)) {
				int i = line.indexOf(":");
				state.headerMap.put(line.substring(0, i), line.substring(i + 1)
						.trim());
			}
		}
	...
	private void doResponse() {
		...
		int contentLength = tmp == null ? (raw.size() - state.headerLength)
	...
	private void doHttp() {
		byte[] data = raw.toByteArray();
		byte[] contentRaw = Arrays.copyOfRange(data, state.headerLength,
				data.length);
					...
					try {
						for (int i = cur; i < contentRaw.length - 1; i++) {
							if (contentRaw[i] == '\r'
								...
								for (byte b : Arrays.copyOfRange(contentRaw,
										cur, i)) {
									chunkSize <<= 4;
									chunkSize |= b > '9' ? ((b | 32) - ('a' - 10))
											: (b - '0');
								}
								i += 2;
								if (chunkSize > 0) {
									if (contentRaw.length < i + chunkSize) {
										// 数据还不够
										state.length += i + chunkSize
												- contentRaw.length;
										return;
									}
									buff.write(Arrays.copyOfRange(contentRaw,
											i, i + chunkSize));
								}
								cur = chunkSize + i + 2;
								i = cur - 1;
	...
	protected void doHttp(Request request, Response response) {
		...
		if (response.content != null) {
			if (type != null) {
				if (type.startsWith("text") || type.indexOf("html") >= 0
						|| type.indexOf("json") >= 0
						|| type.indexOf("javascript") >= 0) {
					System.out.println("\t"
							+ new String(response.content).replaceAll(
									"\\n|\\r", "\\\\n"));
				} else {
