...
public class PcapBpfProgram
{
    ...
    static
    {
         ...
         * before us, otherwise we could get UnsatisfiedLinkError from JNI
         * runtime if we're invoked before Pcap class.
         */
        try
        {
     ...
     * initialization. The object is only suitable for passing to Pcap.compile
     * or Pcap.compileNoPcap which will initiliaze it. Using any of the getter
    ...
    public PcapBpfProgram(byte[] instructions)
    {
        ...
        /*
         * Allocate bpf_program structure in native memory and copy the byte
         * array
         */
    ...
    public PcapBpfProgram(ByteBuffer instructions)
    {
        ...
        else
        {
            ...
            /*
             * We need to make sure we keep a reference to the buffer so it
             * doesn't get GCed since we're referencing its memory from peered
    ...
    /**
     * Cleans up JNI resources and releases any unreleased BPF programs in
     * native land.
     */
    @Override
    ...
    /**
     * Allocates new bpf_program structure and enough space for code in the
     * array and makes a copy.
     * 
     ...
     * content are also directly chaged in memory. Therefore care should be
     * taken.
     * 
