...
public class TextFormatter extends JFormatter
{
    ...
    protected void fieldAfter(JHeader header, JField field, Detail detail) throws IOException
    {
        ...
        else if (field.getStyle() != Style.BYTE_ARRAY_HEX_DUMP && field.getStyle() != Style.STRING_TEXT_DUMP)
        {
            decLevel();
        }
    }
    ...
    protected void fieldBefore(JHeader header, JField field, Detail detail) throws IOException
    {
        ...
        else if (field.getStyle() == Style.INT_BITS)
        {
            ...
            pad().format("%s = [%d] %s%s", v, i, field.getDisplay(header), ((d == null) ? "" : ": " + d));
        ...
        else if (field.getStyle() == Style.STRING_ARRAY)
        {
            ...
            for (String b : table)
            {
                pad().format(FIELD_ARRAY_FORMAT + "%s", field.getDisplay(header), i++, b);
            }
    ...
    protected void headerBefore(JHeader header, Detail detail) throws IOException
    {
        ...
        {
            pad().format(" ******* %s - \"%s\" - offset=%d (0x%X) length=%d %s", header.getName(), header.getDescription(), header.getOffset(),
                    header.getOffset(), header.getLength(), suiteStr);
        }
        else
        {
            pad().format(" ******* %s offset=%d (0x%X) length=%d %s", header.getName(), header.getOffset(), header.getOffset(), header.getLength(), suiteStr);
        }
        pad();
    ...
    public void packetBefore(JPacket packet, Detail detail) throws IOException
    {
        ...
        {
            pad().format(FIELD_FORMAT + "%d", "number", packet.getState().getFrameNumber());

        }

        pad().format(FIELD_FORMAT + "%s", "timestamp", new Timestamp(packet.getCaptureHeader().timestampInMillis()).toString());

        pad().format(FIELD_FORMAT + "%d bytes", "wire length", packet.getCaptureHeader().wirelen());
        pad().format(FIELD_FORMAT + "%d bytes", "captured length", packet.getCaptureHeader().caplen());
    ...
    protected void subHeaderBefore(JHeader header, JHeader subHeader, Detail detail) throws IOException
    {
        ...
        pad().format("+ %s: offset=%d length=%d", subHeader.getName(), subHeader.getOffset(), subHeader.getLength());
    }
}
