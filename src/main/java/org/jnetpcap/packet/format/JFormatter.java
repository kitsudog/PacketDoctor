...
public abstract class JFormatter
{
    ...
    public enum Style {
         ...
         * The BYT e_ arra y_ he x_ dum p_ n o_
         * address.
         */
         ...
         * The BYT e_ arra y_ he x_ dum p_ n o_ tex
         * t_ address.
         */
     ...
     * 
     * @param enable true will enable display of payload header otherwise
     *        disable
     ...
     */
    protected abstract void fieldAfter(JHeader header, JField field, Detail detail) throws IOException;
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    protected abstract void fieldBefore(JHeader header, JField field, Detail detail) throws IOException;
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    public void format(JHeader header, JField field, Detail detail) throws IOException
    {
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    public void format(JHeader header, JHeader subHeader, Detail detail) throws IOException
    {
     ...
     * @throws IOException any IO errors when sending data to default output
     *         device
     */
    public void format(JPacket packet, Detail detail) throws IOException
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            try
            {
                ...
                final Detail headerDetail = (detailsPerHeader[id] == null) ? detail : detailsPerHeader[id];
    ...
    private String formatIpAddress(byte[] address)
    {
        ...
        return (address.length == 16) ? FormatUtils.asStringIp6(address, true) : FormatUtils.asString(address, '.', 10).toUpperCase();
    }
    ...
    private String formatMacAddress(byte[] address)
    {
        ...
        if (resolveAddresses && ouiPrefixResolver.canBeResolved(address))
        {
            ...
            String s = prefix + "_" + FormatUtils.asStringZeroPad(address, ':', 16, 3, 3).toLowerCase();
            return s + " (" + f + ")";
        }
     ...
     * @throws IOException any IO errors while sending data to output device
     */
    protected abstract void headerAfter(JHeader header, Detail detail) throws IOException;
     ...
     * @throws IOException any IO errors while sending data to output device
     */
    protected abstract void headerBefore(JHeader header, Detail detail) throws IOException;
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    public abstract void packetAfter(JPacket packet, Detail detail) throws IOException;
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    public abstract void packetBefore(JPacket packet, Detail detail) throws IOException;
    ...
    {
        String f = (address.length == 16) ? FormatUtils.asStringIp6(address, true) : FormatUtils.asString(address, '.', 10).toUpperCase();
     ...
     * 
     * @param enable if true will include payload header in the display,
    ...
    public void setResolveAddresses(boolean enable)
    {
        ...
        {
            this.ouiPrefixResolver = JRegistry.getResolver(ResolverType.IEEE_OUI_PREFIX);
     ...
     * @return the string[]
     */
    protected String[] stylizeMultiLine(JHeader header, JField field, Style style, Object value)
    {

        switch (style)
        {
            case BYTE_ARRAY_HEX_DUMP:
                return FormatUtils.hexdump((byte[]) value, header.getOffset(), 0, true, true, true);

            case BYTE_ARRAY_HEX_DUMP_NO_TEXT:
                return FormatUtils.hexdump((byte[]) value, header.getOffset(), 0, true, false, true);

            case BYTE_ARRAY_HEX_DUMP_NO_TEXT_ADDRESS:
                return FormatUtils.hexdump((byte[]) value, header.getOffset(), 0, false, false, true);

            case BYTE_ARRAY_HEX_DUMP_NO_ADDRESS:
                return FormatUtils.hexdump((byte[]) value, header.getOffset(), 0, false, true, true);

            case BYTE_ARRAY_HEX_DUMP_ADDRESS:
                return FormatUtils.hexdump((byte[]) value, header.getOffset(), 0, true, false, false);

            case BYTE_ARRAY_HEX_DUMP_TEXT:
                return FormatUtils.hexdump((byte[]) value, header.getOffset(), 0, false, true, false);
            ...
            default:
                return new String[]
                { stylizeSingleLine(header, field, value) };
        }
    }
    ...
    protected String stylizeSingleLine(JHeader header, JField field, Object value)
    {
        ...
        switch (style)
        {
            ...
            case INT_HEX:
                return "0x" + Long.toHexString(((Number) value).longValue()).toUpperCase() + " (" + value.toString() + ")";

            case LONG_HEX:
                return "0x" + Long.toHexString((long) (Long) value).toUpperCase() + " (" + value.toString() + ")";
