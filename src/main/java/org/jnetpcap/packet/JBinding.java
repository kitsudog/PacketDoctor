...
public interface JBinding extends JDependency
{
    ...
    public static abstract class DefaultJBinding implements JBinding
    {
         ...
         * @param targetId ID of the header to which this binding needs to be
         *        applied to. The target ID also becomes an automatic dependency
         *        since that is always the case.
         * @param dependencyIds additional IDs of headers that are referenced in
        ...
        public DefaultJBinding(int myId, int targetId, int... dependencyIds)
        {
            ...
            System.arraycopy(dependencyIds, 0, this.dependencyIds, 1, dependencyIds.length);
     ...
     * @param offset offset into the packet data buffer where the end of the
     *        previous header is
     * @return either full or truncated length of the header or 0 if header is
     *         not bound at all
     */
