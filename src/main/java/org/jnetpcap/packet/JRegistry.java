...
public final class JRegistry
{
    ...
    static
    {
        ...
        for (JProtocol p : JProtocol.values())
        {
            ...
            try
            {
                JBinding[] bindings = AnnotatedBinding.inspectJHeaderClass(p.getHeaderClass(), errors);
    ...
    public static void addBindings(Class<?> container)
    {
        ...
        {
            addBindings(AnnotatedBinding.inspectJHeaderClass((Class<? extends JHeader>) container, errors));
     ...
     * 
     * @param classes classes of all the headers that java scanner will be
     *        cleared if previously registered
     */
    ...
    {
        return dlt >= 0 && dlt < DLTS_TO_IDS.length && DLTS_TO_IDS[dlt] != NO_DLT_MAPPING;
    }
     ...
     */
    public static AnnotatedHeader inspect(Class<? extends JHeader> c, List<HeaderDefinitionError> errors)
    {
     ...
     */
    public static AnnotatedHeader lookupAnnotatedHeader(Class<? extends JHeader> c) throws UnregisteredHeaderException
    {
     ...
     * @throws UnregisteredHeaderException the unregistered header exception
     */
    public static AnnotatedHeader lookupAnnotatedHeader(int id) throws UnregisteredHeaderException
    {
        if (MAP_BY_ID[id] == null || MAP_BY_ID[id].annotatedHeader == null)
        {
            throw new UnregisteredHeaderException("header [" + id + "] not registered");
        }
    ...
    static AnnotatedHeader lookupAnnotatedSubHeader(Class<? extends JSubHeader<? extends JSubHeader<?>>> c)
    {
        ...
        {
            throw new UnregisteredHeaderException("sub header [" + c.getName() + "] not registered, most likely parent not registered as well");
        }
     ...
     * @throws UnregisteredHeaderException thrown if protocol not found, invalid
     *         ID
     */
    public static Class<? extends JHeader> lookupClass(int id) throws UnregisteredHeaderException
    {
     ...
     * @throws UnregisteredHeaderException the unregistered header exception
     */
    private static int lookupIdNoCreate(Class<? extends JHeader> c) throws UnregisteredHeaderException
    {
        if (mapByClassName.containsKey(c.getCanonicalName()) == false)
        {
            throw new UnregisteredHeaderException("header [" + c.getName() + "] not registered");
        }
     ...
     */
    public static int register(Class<? extends JHeader> c) throws RegistryHeaderErrors
    {
        ...
        {
            throw new RegistryHeaderErrors(c, errors, "while trying to register " + c.getSimpleName() + " class");
        }
     ...
     */
    public static int register(Class<? extends JHeader> c, List<HeaderDefinitionError> errors)
    {
    ...
    public static void setScanners(Class<?> c)
    {
        ...
        {
            setScanners(AnnotatedScannerMethod.inspectJHeaderClass((Class<? extends JHeader>) c));
        }
        else
        {
            setScanners(AnnotatedScannerMethod.inspectClass((Class<? extends JHeader>) c));
        }
    }
     ...
     * Sets the header scanners for each header type and protocol.
     * 
     * @param container the new header scanners for each header type and
     *        protocol
     */
    ...
    public static String toDebugString()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < A_MAX_ID_COUNT; i++)
            {
                if (scanners[i] != null)
                {
                    out.format("scanner[%-2d] class=%-15s %s\n", i, lookupClass(i).getSimpleName(), scanners[i].toString());
                }
            }
            ...
            for (int i = 0; i < MAX_DLT_COUNT; i++)
            {
                if (hasDltMapping(i))
                {
                    ...
                    out.format("libpcap::%-24s => header::%s.class(%d)\n", PcapDLT.valueOf(i).toString() + "(" + i + ")", c.getSimpleName(), id);
                }
            }
        }
