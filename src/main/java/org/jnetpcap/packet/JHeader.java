...
public abstract class JHeader extends JBuffer implements JPayloadAccessor
{
    ...
    public static class State extends JStruct
    {
        ...
        /**
         * Flag set in the header_t structure, tells if the CRC, if performed,
         * was valid or invalid. Invalid CRC means that computed CRC did not
         * match the protocol specific CRC stored within the header.
         */
        ...
        /**
         * Flag set in the header_t structure, which tells if this header was
         * bound due to a heuristic binding. If set, this means that the header
         * was found heuristically (guessed based on correctness of the field
         * values within the header). It is always possible, that heuristic
         * binding may be incorrect and this flag, provides a way to determine
         * how trust worthy the binding is.
         */
         ...
         * protocols with a payload buffer of static length would set this flag
         * or if the payload buffer is at the end of the packet and a
         * calculation can determine based on over-all packet trancation, if the
         * payload was affected as well.
         */
         ...
         * header and/or payload are fragmented. The header fragmentation could
         * have been determined by the scanner of this protocol or the flag
         * could have been inherited from encapsulating header.
         */
        ...
        /**
         * Flag set in the header_t structure, which indicates that the current
         * header was dissected for optional sub-headers. All optional
         * sub-headers were recorded.
         */
         ...
         * header boundaries. This is used in cases such as encapsulated IP and
         * ICMP header, where the original IP packet was truncated before
         * encapsulating in ICMP, yet its header tot_length was not adjusted.
         */
        ...
        /**
         * Gets the length in bytes of a gap (padding) between the header and
         * the protocol 'records' payload:
         * 
         * <pre>
         * +------------------=====--------------------+
        ...
        /**
         * Gets the numerical ID of the header this structure describes as
         * defined by <code>JRegistry</code>.
         * 
        ...
        /**
         * Gets the length of the protocol's header in bytes within the
         * protocol's 'record':
         * 
         * <pre>
         * +---------========--------------------------+
        ...
        /**
         * Gets the length of the payload that follows a protocol header in
         * bytes within the protocol's 'record':
         * 
         * <pre>
         * +------------------------=========----------+
         * | prefix | header | gap | PAYLOAD | postfix |
        ...
        /**
         * Gets the length of the postfix that follows a protocol's payload in
         * bytes within the protocol's 'record':
         * 
         * <pre>
         * +----------------------------------=========+
        ...
        /**
         * Gets the length of the prefix that precedes a protocol's header in
         * bytes within the protocol's 'record':
         * 
         * <pre>
         * +========-----------------------------------+
        ...
        /**
         * Peers this state object with the native structures of another. No
         * copies are done, only references are changed.
         * 
         * @param peer destination object holding the native memory reference we
         *        need to peer to
         ...
         * 
         * @param flags unsinged integer containing the bit-flags to set for
         *        this header
         */
    ...
    /**
     * A constant that defines how many bits there are in a byte. This constant
     * is used in unit conversion from bytes to bits and visa-versa. To convert
     * from bytes to bit you multiple your number of bytes by the BYTE constant.
     * To convert from bits to bytes, you divide the number of bits by the BYTE
     * constant.
     */
    ...
    /**
     * Gets the size of the native header_t structure on this particular
     * platform.
     * 
     ...
     * of the header. The method invoked must be defined in the header
     * definition otherwise an exception will be thrown.
     */
    public JHeader()
    {
        super(Type.POINTER);
     ...
     * @param fields fields usd by the formatter to reformat the packet for
     *        output
    ...
    /**
     * Method that gets called everytime a header is successfully peered with
     * new buffer and/or state structure. This method in JHeader is empty and is
     * expected to be overriden by subclasses of JHeader that require special
     * processing of the header such as decoding its structure at runtime when
     * the header object is bound to new state.
     */
    ...
    /**
     * Retrives the description property for this header as defined in the
     * source definition.
     * 
     ...
     * used by a formatter.
     * 
     * @return an array of fields that this header is made up of, as determined
     *         at runtime
     */
    ...
    /**
     * Reads the contents of the protocol's 'record' gap property as a byte
     * array.
     * 
     * <pre>
     * +------------------=====--------------------+
     ...
     * Even if the gap is zero length or not set, the offset is still calculated
     * and will always be the first byte past the header.
     * 
     * <pre>
     * +------------------=====--------------------+
     * | prefix | header | GAP | payload | postfix |
    ...
    /**
     * Convenience method that retrieves the contents of the header as a byte
     * are
     * 
     * <pre>
     * +---------========--------------------------+
    ...
    /**
     * Gets the numerical ID of this protocol header at runtime as assigned by
     * the JRegistry.
     * 
    ...
    /**
     * If this is a sub-header of another header, gets the reference to the
     * parent header.
     * 
    ...
    /**
     * The offset in bytes into the packet buffer where the prefix starts, even
     * if prefix is not set or zero in length.
     * 
     * @return offset in bytes into the packet buffer
     */
    ...
    /**
     * Gets the header's index into the packet state structure. Various pieces
     * of information in packet state structure can be looked up using this
     * index.
     * 
