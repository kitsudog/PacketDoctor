 ...
 * </pre>
 * 
 * <b>Note</b> that scan_t structure is implemented by java class JScan which is
 * peered with that structure.
 * </p>
 * 
 * @see JScan
...
public class JHeaderScanner extends JFunction
{
    ...
    /**
     * A java scanner for headers out of a native packet buffer. This
     * constructor allows a custom header scanner to be implemented and
     * registered with JRegistry. The packet scanner, JScanner, uses builtin
     * native scanners to scan packet buffers but also allows custom java
     * scanners to override or provide additional header scanners. Any new
     * protocol header being added to jNetPcap library of protocols, that is not
     * officially released with this API, will have to provide its own custom
     * header scanner.
     * 
     * @param protocol core protocol constant for which to override its default
     ...
     */
    private AnnotatedHeaderLengthMethod getLengthMethod(final HeaderLength.Type type)
    {
     ...
     * @param offset offset into the packet buffer in bytes of the start of this
     *        header
     * @return length of the header or 0 if this header is not found in the
     *         packet buffer
     */
    public int getHeaderLength(final JPacket packet, final int offset)
    {
        return getLengthMethod(HeaderLength.Type.HEADER).getHeaderLength(packet, offset);
    }
    ...
    {
        return (getLengthMethod(HeaderLength.Type.PREFIX) == null) ? 0 : getLengthMethod(HeaderLength.Type.PREFIX).getHeaderLength(packet, offset);
    }
    ...
    {
        return (getLengthMethod(HeaderLength.Type.GAP) == null) ? 0 : getLengthMethod(HeaderLength.Type.GAP).getHeaderLength(packet, offset);
    }
    ...
    {
        return (getLengthMethod(HeaderLength.Type.PAYLOAD) == null) ? 0 : getLengthMethod(HeaderLength.Type.PAYLOAD).getHeaderLength(packet, offset);
    }
    ...
    {
        return (getLengthMethod(HeaderLength.Type.POSTFIX) == null) ? 0 : getLengthMethod(HeaderLength.Type.POSTFIX).getHeaderLength(packet, offset);
    }
    ...
    /**
     * Calculates the next header in sequence of headers within the packet
     * buffer.
     * 
     * @param packet the packet object this header is bound to
     * @param offset offset into the packet buffer in bytes of the start of this
     ...
     * 
     * @param scan scan state structure that is used to pass around state both
     *        in java and native user space
     */
    protected void scanHeader(final JScan scan)
    {
        ...
        {
            /*
             * Because java bindings we are about to invoke, rely on the the
             * current header being already recorded in packet_state_t
             * structure, we call on the record_header method menually. Here are
             * some major effects as a result of this call:
             */
            /* 1 - payload is calculated if not set; */
            /*
             * 2 - protocol record properties are truncated from their
             * theoretical;
             */
            /*
            ...
            /*
             * Now call on any custom bindings. If bindings fail, heuristic
             * scanner can still match if its enabled. We need to advance the
             * current offset manually past the header and the gap (between
             * header and payload). Current header is already recorded and can
             * be peered for binding invocation.
             */
            final int next = scanAllBindings(packet, offset + scan.scan_length() + scan.scan_gap());
            scan.scan_next_id(next);

            /*
             * Native main scan loop implements heuristics. Both post and pre
             * heuristic checks are supported. Post heuristic checks are done
             * only after the direct binding checks fail. The above is a "direct
             * binding check", inkoked through normally registered bindings. For
             * pre, we still perform direct checks as normal here, the native
             * scan method, overrides our results. For post, it just checks if
             * we succeeded.
             */
        }
    }
    ...
    private void setAllLengths(final JScan scan, final JPacket packet, int offset)
    {
        ...
        final int prefix = (this.lengthMethods[HeaderLength.Type.PREFIX.ordinal()] == null) ? 0 : this.lengthMethods[HeaderLength.Type.PREFIX.ordinal()]
        ...
        final int header = this.lengthMethods[HeaderLength.Type.HEADER.ordinal()].getHeaderLength(packet, offset);

        final int gap = (this.lengthMethods[HeaderLength.Type.GAP.ordinal()] == null) ? 0 : this.lengthMethods[HeaderLength.Type.GAP.ordinal()]
                .getHeaderLength(packet, offset);

        final int payload = (this.lengthMethods[HeaderLength.Type.PAYLOAD.ordinal()] == null) ? 0 : this.lengthMethods[HeaderLength.Type.PAYLOAD.ordinal()]
                .getHeaderLength(packet, offset);

        final int postfix = (this.lengthMethods[HeaderLength.Type.POSTFIX.ordinal()] == null) ? 0 : this.lengthMethods[HeaderLength.Type.POSTFIX.ordinal()]
                .getHeaderLength(packet, offset);
    ...
    public String toString()
    {
        ...
        out.format("id=%2d, loaded=%-5s direct=%-5s, scan=%-5s bindings=%-2d [%s]", this.id, this.lengthMethods != null, isDirect(), hasScanMethod(),
