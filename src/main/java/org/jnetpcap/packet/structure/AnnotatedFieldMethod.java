...
public abstract class AnnotatedFieldMethod extends AnnotatedMethod
{
    ...
    private static class BooleanFunction extends AnnotatedFieldMethod
    {
         ...
         * 
         * @see
         * org.jnetpcap.packet.AnnotatedFieldMethod#booleanMethod(org.jnetpcap
         * .packet.JHeader)
         */
        /**
         * Boolean method.
        ...
        public final void configFromField(AnnotatedField field)
        {
            ...
            {
                throw new FieldDefinitionException(field, "Missing '" + function.name().toLowerCase() + "' property. [@Dynamic(Property." + function.name()
                        + ")]");
            }
        }
    ...
    private static class IntFunction extends AnnotatedFieldMethod
    {
         ...
         */
        public IntFunction(AnnotatedField field, Field.Property function, int staticValue)
        {
            super(field, function);
        ...
        public final void configFromField(AnnotatedField field)
        {
            ...
            if (hasStaticValue == false && method == null)
            {
                throw new FieldDefinitionException(field, "Missing '" + function.name().toLowerCase() + "' property. [@Field(" + function.name().toLowerCase()
    ...
    private static class LongFunction extends AnnotatedFieldMethod
    {
         ...
         * 
         * @see
         * org.jnetpcap.packet.AnnotatedFieldMethod#longMethod(org.jnetpcap.
         * packet.JHeader)
         */
        /**
         * Long method.
    ...
    private static class ObjectFunction extends AnnotatedFieldMethod
    {
        ...
        public final void configFromField(AnnotatedField field)
        {
            ...
            switch (function)
            {
                case VALUE:
                    if (method == null)
                    {
                        throw new HeaderDefinitionError(field.getDeclaringClass(), "no method set for field value getter [" + field.getName() + "]");
                    }
                    break;

                default:
                    throw new HeaderDefinitionError(field.getDeclaringClass(), "Invalid Dynamic function type " + function.toString());

            }

            if (method == null)
            {
                throw new FieldDefinitionException(field, "Missing field accessor '" + function.name().toLowerCase() + "' property. [@Dynamic(Property."
                        + function.name() + ")]");
            }
        }
         ...
         * 
         * @see
         * org.jnetpcap.packet.AnnotatedFieldMethod#objectMethod(org.jnetpcap
         * .packet.JHeader)
         */
        /**
         * Object method.
    ...
    private static class StringFunction extends AnnotatedFieldMethod
    {
         ...
         * 
         * @see
         * org.jnetpcap.packet.AnnotatedFieldMethod#StringMethod(org.jnetpcap
         * .packet.JHeader)
         */
        /**
         * String method.
    ...
    private static void checkSignature(Method method, Class<?> c)
    {
        ...
        /*
         * Now make sure it has the right signature of: <code>String
         * name()</code.
         */
        final Class<?>[] sig = method.getParameterTypes();
        if ((sig.length == 1 && sig[0] != String.class) || sig.length > 1 || method.getReturnType() != c)
        {
     ...
     */
    public static AnnotatedFieldMethod generateFunction(Field.Property function, AnnotatedField field)
    {
        ...
        switch (function)
        {
            ...
            default:
                throw new HeaderDefinitionError("Unsupported Dynamic function type " + function.toString());
        }
