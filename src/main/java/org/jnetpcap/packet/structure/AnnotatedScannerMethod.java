...
public class AnnotatedScannerMethod extends AnnotatedMethod
{
    ...
    /** The Constant cache. */
    private final static Map<Class<?>, AnnotatedScannerMethod[]> cache = new HashMap<Class<?>, AnnotatedScannerMethod[]>();
     ...
     */
    public static AnnotatedScannerMethod[] inspectJHeaderClass(Class<? extends JHeader> c)
    {
        ...
        {
            AnnotatedScannerMethod[] m = new AnnotatedScannerMethod[]
            { new AnnotatedScannerMethod(ms[0], c) };
            cache.put(c, m);
    ...
    public static AnnotatedScannerMethod[] inspectClass(Class<? extends JHeader> c)
    {
        ...
        List<AnnotatedScannerMethod> list = new ArrayList<AnnotatedScannerMethod>(20);

        for (Method method : getMethods(c, Scanner.class))
        {
            Scanner a = method.getAnnotation(Scanner.class);
            Class<? extends JHeader> clazz = (a.value() == JHeader.class) ? c : a.value();

            if (JHeader.class.isAssignableFrom(c) == false)
            {
                throw new HeaderDefinitionError(c, "non JHeader based classes, " + "must declare protocol class in @Scanner annotation");
            }

            list.add(new AnnotatedScannerMethod(method, clazz));
        }

        AnnotatedScannerMethod[] m = list.toArray(new AnnotatedScannerMethod[list.size()]);
        cache.put(c, m);
     ...
     * @param container the container
     */
    public AnnotatedScannerMethod(Method method, Class<? extends JHeader> c, Object container)
    {
     ...
     * 
     * @see
     * org.jnetpcap.packet.annotate.AnnotatedMethod#validateSignature(java.lang
     * .reflect.Method)
     */
    /**
     * Validate signature.
