...
public class AnnotatedHeaderLengthMethod extends AnnotatedMethod
{
    ...
    /** The Constant cache. */
    private final static Map<Class<?>, AnnotatedHeaderLengthMethod[]> cache = new HashMap<Class<?>, AnnotatedHeaderLengthMethod[]>();
     ...
     *         sub-structures
     */
    public static AnnotatedHeaderLengthMethod[] inspectClass(Class<? extends JHeader> c)
    {
        ...
        AnnotatedHeaderLengthMethod[] methods = new AnnotatedHeaderLengthMethod[HeaderLength.Type.values().length];

        Header header = c.getAnnotation(Header.class);
        if (header != null && header.length() != -1)
        {
            methods[HeaderLength.Type.HEADER.ordinal()] = new AnnotatedHeaderLengthMethod(c, header.length(), HeaderLength.Type.HEADER);
        }

        if (header != null && header.prefix() != -1)
        {
            methods[HeaderLength.Type.PREFIX.ordinal()] = new AnnotatedHeaderLengthMethod(c, header.prefix(), HeaderLength.Type.PREFIX);
        }

        if (header != null && header.gap() != -1)
        {
            methods[HeaderLength.Type.GAP.ordinal()] = new AnnotatedHeaderLengthMethod(c, header.gap(), HeaderLength.Type.GAP);
        }

        if (header != null && header.payload() != -1)
        {
            methods[HeaderLength.Type.PAYLOAD.ordinal()] = new AnnotatedHeaderLengthMethod(c, header.payload(), HeaderLength.Type.PAYLOAD);
        }

        if (header != null && header.postfix() != -1)
        {
            methods[HeaderLength.Type.POSTFIX.ordinal()] = new AnnotatedHeaderLengthMethod(c, header.postfix(), HeaderLength.Type.POSTFIX);
        }
        ...
        for (Method method : getMethods(c, HeaderLength.class))
        {
            ...
            {
                throw new AnnotatedMethodException(c, "duplicate: " + methods[hl.value().ordinal()] + " property and " + method.getName() + "() method");
            }

            checkSignature(method);

            methods[hl.value().ordinal()] = new AnnotatedHeaderLengthMethod(method, hl.value());
        }

        if (methods[HeaderLength.Type.HEADER.ordinal()] == null)
        {
            throw new AnnotatedMethodException(c, "@HeaderLength annotated method not found");
        }
     ...
     * @param type the type
     */
    public AnnotatedHeaderLengthMethod(Class<? extends JHeader> c, int length, HeaderLength.Type type)
    {
    ...
    public int getHeaderLength(JBuffer buffer, int offset)
    {
        ...
        /*
         * Invoke the static method: <code>public static int method(JBuffer,
         * int)</code>
         */
        try
        {
    ...
    private static void checkSignature(Method method)
    {
        ...
        if (t.length != 2 || t[0] != JBuffer.class || t[1] != int.class || method.getReturnType() != int.class)
        {
    ...
    public String toString()
    {
        if (method == null)
        {
            String property = (type == HeaderLength.Type.HEADER) ? "length" : type.toString().toLowerCase();
