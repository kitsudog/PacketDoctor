...
public class AnnotatedHeader
{
    ...
    /** The Constant cache. */
    private final static Map<Class<?>, AnnotatedHeader> cache = new HashMap<Class<?>, AnnotatedHeader>();
    ...
    private static List<Class<?>> getSubHeaderClasses(Class<?> c, String prefix)
    {
        ...
        for (final Class<?> s : c.getClasses())
        {
            ...
            if (s.isAnnotationPresent(Header.class))
            {
                ...
                /*
                 * We're interested in direct sub-header's and not sub-headers
                 * of sub-headers. Again, a sub header has @Header annotation on
                 * it, not just the java declaration class within a class. We're
                 * looking at the hierachy of @Header type sub-headers. Each
                 * sub-headers evaluates its own sub-headers in a seperate scan,
                 * that is why we stop here.
                 */
                continue;
            }
     ...
     *         header name
     */
    private static AnnotatedHeader inspectHeaderAnnotation(Class<? extends JHeader> c, List<HeaderDefinitionError> errors)
    {
        ...
        if (c.isAnnotationPresent(Header.class))
        {
            ...
            if (JHeader.class.isAssignableFrom(c) == false)
            {
                ...
                errors.add(new HeaderDefinitionError(c, "header must subclass 'JHeader'"));
            }
            ...
            {
                for (Class<?> p = c.getEnclosingClass(); p != null; p = p.getEnclosingClass())
                {
                    ...
                    if (p.isAnnotationPresent(Header.class))
                    {
                        ...
                        {
                            errors.add(new HeaderDefinitionError(c, "parentClass header '" + p.getSimpleName() + "' must subclass 'JHeader'"));
                            break;
                        }
        ...
        {
            errors.add(new HeaderDefinitionError(c, "header missing @Header annotation"));
        }
     ...
     * @return the annotated header
     */
    public static AnnotatedHeader inspectJHeaderClass(Class<? extends JHeader> c, List<HeaderDefinitionError> errors)
    {
        ...
        /*
         * Prepare by extracting all annotated methods and putting them into
         * their own buckets
         */
        ...
        Map<String, AnnotatedField> fields = new HashMap<String, AnnotatedField>(fieldMethods.size());
        ...
        for (Method m : fieldMethods)
        {
            try
            {
                ...
                {
                    throw new HeaderDefinitionError(c, "duplicate field " + field.getName());
                }
        ...
        Map<Property, AnnotatedFieldMethod> defaultMethods = new HashMap<Property, AnnotatedFieldMethod>();
        ...
        for (Method m : runtimeMethods)
        {
            try
            {
                ...
                if (field == null)
                {
                    throw new HeaderDefinitionError(c, "runtime can not find field " + function.getFieldName());
                }
         ...
         * constant properties and never are field value getter methods
         * themselves. Therefore the default dynamic methods must have already
         * been seen and defined in the class hierarchy somewhere, otherwise its
         * an error.
         * 
         * <pre>
         * &#064;Field 
        ...
        for (Class<?> e : c.getClasses())
        {
            ...
            if (e.isAnnotationPresent(Field.class) && e.isEnum())
            {
                ...
                for (Object element : e.getEnumConstants())
                {
                    ...
                    try
                    {
                        AnnotatedField field = AnnotatedField.inspectEnumConstant(name, enumAnnotation, defaultMethods, c);
        ...
        for (Iterator<AnnotatedField> i = fields.values().iterator(); i.hasNext();)
        {
            ...
            try
            {
                ...
                {
                    throw new HeaderDefinitionError(c, "invalid parentClass name for sub-field " + field.getName());
                }

                AnnotatedField parent = fields.get(field.getParent());
                if (parent == null)
                {
                    throw new HeaderDefinitionError(c, "can not find parentClass '" + field.getParent() + "' for sub field '" + field.getName() + "'");
                }
        ...
        /*
         * Last, tell all the fields we are done processing and let them finish
         * up whatever they need to
         */
         ...
         * hierachy looking for classes marked with @Header annotation. Unless
         * they have the "parentClass" parameter defined, they automatically
         * become the sub-header of us. The sub-class must also extend
         * JSubHeader class, if it doesn't we skip it and report an error.
         */
        List<Class<?>> subClasses = getSubHeaderClasses(c, c.getSimpleName());
        List<AnnotatedHeader> subHeaders = new ArrayList<AnnotatedHeader>(subClasses.size());
        for (Class<?> s : subClasses)
        {
            ...
            {
                errors.add(new HeaderDefinitionError(c, "skipping sub-header " + s.getSimpleName() + ". The sub-header must subclass JSubHeader class"));
                continue;
            }
            ...
            subHeaders.add(inspectJHeaderClass(s.asSubclass(JSubHeader.class), errors));

        }

        header.saveSubHeaders(subHeaders.toArray(new AnnotatedHeader[subHeaders.size()]));

        header.saveFields(fields.values().toArray(new AnnotatedField[fields.size()]));
