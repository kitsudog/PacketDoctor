...
public class Rtp extends JHeader
{
     ...
     * mechanism is designed so that the header extension may be ignored by
     * other interoperating implementations that have not been extended.
     * Schulzrinne, et al. Standards Track [Page 18] RFC 3550 RTP July 2003 Note
     * that this header extension is intended only for limited use. Most
     * potential uses of this mechanism would be better done another way, using
     * the methods described in the previous section. For example, a
     * profile-specific extension to the fixed header is less expensive to
     * process because it is not conditional nor in a variable location.
     * Additional information required for a particular payload format SHOULD
     * NOT use this header extension, but SHOULD be carried in the payload
     ...
     * extension header (therefore zero is a valid length). Only a single
     * extension can be appended to the RTP data header. To allow multiple
     * interoperating implementations to each experiment independently with
     * different header extensions, or to allow a particular implementation to
     * experiment with more than one type of header extension, the first 16 bits
     * of the header extension are left open for distinguishing identifiers or
     * parameters. The format of these 16 bits is to be defined by the profile
     ...
     * <p>
     * This is a baseclass, suitable for use by extending it with the
     * appropriate extension based on the profile definition. The class defines
     * methods for reading the static fields that are present in every
     * extension, especially the {@link #length} field. No sub-header ID needs
     * to be manually assigned since Rtp specification only allows a single
     * extension to exist within a header. Note that the profile specific 16-bit
     * field has a getter method provided but is not marked with
     * <code>Field</code> annotation. This provides a method for reading the raw
     * 16-bit field value and allows the subclass to provide its own method that
     * is profile specific and which possibly further breaks down the structure
     * of this raw field into a sub-structure.
     * </p>
     * 
     * @author Mark Bednarczyk
     * @author Sly Technologies, Inc.
    ...
    public abstract static class Extension extends JSubHeader<Rtp>
    {
        ...
        /**
         * Determines the length of the header in octets. The value is
         * calculated by use of a 16-bit length field that counts the number of
         ...
         * extension, excluding the four-octet extension header (therefore zero
         * is a valid length).
         * 
         * @return length of the extension header in 32-bit words, minus the 4
         *         byte static part
         */
    ...
    /**
     * Constant containing the name of the RFC that describes the specification
     * of this header.
     */
    ...
    /**
     * Constant containing the name of the protocol suite this header belongs
     * to.
     */
    ...
    /**
     * Bitmask applied to byte 0 in the header which masks off the version
     * number of the Rtp header within the packet.
     */
    ...
    public static int headerLength(final JBuffer buffer, final int offset)
    {
        ...
        {
            return rtpBaseHeader + Rtp.Extension.headerLength(buffer, offset + rtpBaseHeader);
        }
        else
        {
    ...
    /**
     * Determines the length of Rtp padding if the header has been padded. If
     * the Rtp.P bit is set, that means that last byte within the frame contains
     * the number of bytes that were used to pad after the payload following
     * this header.
     * 
     ...
     * identified. CSRC identifiers are inserted by mixers (see Section 7.1),
     * using the SSRC identifiers of contributing sources. For example, for
     * audio packets the SSRC identifiers of all sources that were mixed
     * together to create a packet are listed, allowing correct talker
     ...
     * type field (see Section 5.3 of RFC3550).
     * 
    ...
    public int paddingLength()
    {
        ...
        final int length = this.packet.getUByte(getPostfixOffset() + getPostfixLength() - 1);
     ...
     * An example algorithm for generating a random identifier is presented in
     * Appendix A.6. Although the probability of multiple sources choosing the
     * same identifier is low, all RTP implementations must be prepared to
     * detect and resolve collisions. Section 8 describes the probability of
     * collision along with a mechanism for resolving collisions and detecting
     * RTP-level forwarding loops based on the uniqueness of the SSRC
     * identifier. If a source changes its source transport address, it must
     * also choose a new SSRC identifier to avoid being interpreted as a looped
     ...
     * and jitter calculations (see Section 6.4.1). The resolution of the clock
     * MUST be sufficient for the desired synchronization accuracy and for
     * measuring packet arrival jitter (one tick per video frame is typically
     * not sufficient). The clock frequency is dependent on the format of data
     ...
     * timestamps if they are (logically) generated at once, e.g., belong to the
     * same video frame. Consecutive RTP packets MAY contain timestamps that are
     * not monotonic if the data is not transmitted in the order it was sampled,
     * as in the case of MPEG interpolated video frames. (The sequence numbers
     * of the packets as transmitted will still be monotonic.) </p> </p> RTP
     * timestamps from different media streams may advance at different rates
     ...
     * directly comparing RTP timestamps from different media is not effective
     * for synchronization. Instead, for each medium the RTP timestamp is
     * related to the sampling instant by pairing it with a timestamp from a
     ...
     * as described in Section 6.4. </p> </p> The sampling instant is chosen as
     * the point of reference for the RTP timestamp because it is known to the
     * transmitting endpoint and has a common definition for all media,
     * independent of encoding delays or other processing. The purpose is to
     * allow synchronized presentation of all media sampled at the same time.
     * </p> </p> Applications transmitting stored data rather than data sampled
     * in real time typically use a virtual presentation timeline derived from
     * wallclock time to determine when the next frame or other unit of each
     * medium in the stored data should be presented. In this case, the RTP
     * timestamp would reflect the presentation time for each unit. That is, the
     * RTP timestamp for each unit would be related to the wallclock time at
     * which the unit becomes current on the virtual presentation timeline.
     * Actual presentation occurs some time later as determined by the receiver.
     * </p> </p> An example describing live audio narration of prerecorded video
