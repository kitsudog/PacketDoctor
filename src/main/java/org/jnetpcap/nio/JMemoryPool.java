...
public class JMemoryPool
{
    ...
    /**
     * Transfers contents from src1 and src2 to a contiguous block of new
     * memory, then peers dst1 and dst2 with the new memory, using the same
     * sizes as src1 and src2 respectively. This operation combines memory
     * allocation, transferTo call on src1 and src2 and then peering of dst1 and
     * dst2 with new memory in a single step.
     * 
     ...
     * @param dst2 peered with same memory at src1.length offset using src2
     *        length as length of peer
     ...
     * then peers dst1 and dst2 with the new memory, using the same sizes as
     * src1 and src2 respectively. This operation combines memory allocation,
     * transferTo call on src1 and src2 and then peering of dst1 and dst2 with
     * new memory in a single step.
     * 
     * @param src1 first src for duplicate
     * @param src2 second src for duplicate into the same memory
     * @param dst1 peered with new memory using src1 length
     ...
     * @return total number of bytes duplicated
     */
    public synchronized int duplicate2(JMemory src1, ByteBuffer src2, JMemory dst1, JMemory dst2)
    {
     ...
     *         will then return an offset into the memory which has been
     *         reserved for this allocation. The pool of used blocks with
     *         potential of some available memory in them is maintained using a
     *         WeakReference. This allows the blocks to be GCed when no
     *         references to them exist, even if there is still a bit of
    ...
    {
        return new Block((atLeastInSize > this.blockSize) ? atLeastInSize : this.blockSize);
    }
    ...
    public int getBlockSize()
    {
        ...
        if (s != null)
        {
            blockSize = (int) JMemory.parseSize(s); // process suffixes
                                                    // kb,mb,gb,tb
        }
