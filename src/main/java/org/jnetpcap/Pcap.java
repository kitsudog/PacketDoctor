 ...
 * 
 * Pcap pcap = Pcap.openLive(netInterface.getName(), snaplen, promiscous, timeout, errbuf);
 * </pre>
 * 
 ...
 * {
 * 
 *     public void newPacket(PrintStream out, int caplen, int len, int seconds, int usecs, ByteBuffer buffer)
 *     {
 * 
...
public class Pcap
{
     ...
     * uses LOOP_INFINATE for something similar, but definately not identical to
     * this option.
     */
     ...
     * 
     * @deprecated The constant name is miss-spelled. You should use the
     *             correctly spelled constant LOOP_INFINITE
     * @see #LOOP_INFINITE
     */
    @Deprecated
    ...
    static
    {
        ...
        try
        {
            ...
            // we hold a Global reference to the class once initialized in JNI,
            // will
            // never be unloaded
     ...
     * without necessarily having called pcap_open(). (pcap_compile_nopcap() is
     * a wrapper around pcap_open_dead(), pcap_compile(), and pcap_close(); the
     ...
     * </p>
     * 
     * @param snaplen generate code to truncate packets to this length upon a
     *        match
     * @param dlt the first header type within the packet, or data link type of
     *        the interface
     * @param program initially empty, but after the method call will contain
     ...
     */
    public native static int compileNoPcap(int snaplen, int dlt, PcapBpfProgram program, String str, int optimize, int netmask);
     ...
     * </ul>
     * 
     * @param alldevs the list is filled in with <code>PcapIf</code> interface
     *        objects; the list must not be immutable
     * @param errbuf error buffer containing error message as a string on
     *        failure
     ...
     * @since 1.2
     */
    public native static int findAllDevs(List<PcapIf> alldevs, StringBuilder errbuf);
     ...
     * 
     * @param alldevs is set to point to the first element of the list; each
     *        element of the list is of type PcapIf
     * @param errbuf error buffer containing error message as a string on
     *        failure
     ...
     * <code>lookupNet</code>.
     * 
     * @param errbuf if there is an error, errbuf is filled with appropriate
     *        message
     ...
     */
    public native static int lookupNet(String device, JNumber netp, JNumber maskp, StringBuilder errbuf);
     ...
     */
    @Deprecated
    public native static int lookupNet(String device, PcapInteger netp, PcapInteger maskp, StringBuilder errbuf);
     ...
     * @since 1.2
     */
    public native static Pcap openLive(String device, int snaplen, int promisc, int timeout, StringBuilder errbuf);
     ...
     * 
     * @throws PcapClosedException if pcap_t structure has been deallocated,
     *         another words if Pcap.close has already been called.
     */
     ...
     */
    public native int compile(PcapBpfProgram program, String str, int optimize, int netmask);
     ...
     * @return the int
     */
    private native <T> int dispatch(int cnt, ByteBufferHandler<T> handler, T user, PcapHeader header);
    ...
    public <T> int dispatch(int cnt, int id, JPacketHandler<T> handler, T user)
    {
        ...
        return dispatch(cnt, id, handler, user, packet, packet.getState(), packet.getCaptureHeader(), JScanner.getThreadLocal());
    }
     ...
     * @return the int
     */
    private native <T> int dispatch(int cnt, int id, JPacketHandler<T> handler, T user, JPacket packet, JPacket.State state, PcapHeader header, JScanner scanner);
     ...
     * @return the int
     */
    private native <T> int dispatch(int cnt, int id, PcapPacketHandler<T> handler, T user, JPacket packet, JPacket.State state, PcapHeader header,
            JScanner scanner);
    ...
    {
        return dispatch(cnt, handler, user, new PcapHeader(Type.POINTER), new JBuffer(Type.POINTER));
    }
     ...
     * @return the int
     */
    private native <T> int dispatch(int cnt, JBufferHandler<T> handler, T user, PcapHeader header, JBuffer buffer);
    ...
    {
        final PcapPacket packet = new PcapPacket(Type.POINTER);
        return dispatch(cnt, datalinkToId(), handler, user, packet, packet.getState(), packet.getCaptureHeader(), JScanner.getThreadLocal());
    }
     ...
     * @return number of packet captured
     */
    public <T> int dispatch(int cnt, JPacketHandler<T> handler, T user, JScanner scanner)
    {
        final PcapPacket packet = new PcapPacket(Type.POINTER);
        return dispatch(cnt, datalinkToId(), handler, user, packet, packet.getState(), packet.getCaptureHeader(), scanner);
    }
     ...
     * @param handler called when packet arrives for each packet
     * @param user opaque user object
     * @param scanner a custom user quick-scanner for parsing headers within the
     *        packet
     * @return 0 on success, -1 on error and -2 if breakloop was used interrupt
     *         the captue
     * @deprecated user of PcapHandler has been replaced with ByteBufferHandler
     ...
     */
    @Deprecated
    public <T> int dispatch(int cnt, PcapPacketHandler<T> handler, T user, JScanner scanner)
    {
        final PcapPacket packet = new PcapPacket(Type.POINTER);
        return dispatch(cnt, datalinkToId(), handler, user, packet, packet.getState(), packet.getCaptureHeader(), scanner);
    }
     ...
     * 
     * @param fname specifies the name of the file to open; currently the
     *        libpcap option to open stdout by using "-" as a string, is not
     ...
     * @return the int
     */
    private native <T> int loop(int cnt, ByteBufferHandler<T> handler, T user, PcapHeader header);
    ...
    {
        final PcapPacket packet = new PcapPacket(Type.POINTER);
        return loop(cnt, id, handler, user, packet, packet.getState(), packet.getCaptureHeader(), JScanner.getThreadLocal());
    }
     ...
     * @return the int
     */
    private native <T> int loop(int cnt, int id, JPacketHandler<T> handler, T user, JPacket packet, JPacket.State state, PcapHeader header, JScanner scanner);
     ...
     * @return the int
     */
    private native <T> int loop(int cnt, int id, PcapPacketHandler<T> handler, T user, JPacket packet, JPacket.State state, PcapHeader header, JScanner scanner);
     ...
     * @return the int
     */
    private native <T> int loop(int cnt, JBufferHandler<T> handler, T user, PcapHeader header, JBuffer buffer);
    ...
    {
        final PcapPacket packet = new PcapPacket(Type.POINTER);
        return loop(cnt, datalinkToId(), handler, user, packet, packet.getState(), packet.getCaptureHeader(), JScanner.getThreadLocal());
    }
     ...
     *         the captue
     */
    public <T> int loop(int cnt, JPacketHandler<T> handler, T user, JScanner scanner)
    {
        final PcapPacket packet = new PcapPacket(Type.POINTER);
        return loop(cnt, datalinkToId(), handler, user, packet, packet.getState(), packet.getCaptureHeader(), scanner);
    }
     ...
     *         the captue
     */
    public <T> int loop(int cnt, PcapPacketHandler<T> handler, T user, JScanner scanner)
    {
        final PcapPacket packet = new PcapPacket(Type.POINTER);
        return loop(cnt, datalinkToId(), handler, user, packet, packet.getState(), packet.getCaptureHeader(), scanner);
    }
     ...
     * 
     * @param pkt_header a packet header that will be initialized to
     *        corresponding C structure captured values
     * @param buffer a buffer that will be peered with returned buffer from
     *        libpcap
     ...
     * </pre>
     * 
     * @param packet packet that will be peered with the next libpcap returned
     *        packet
     * @return the status code
     *         <ul>
    ...
    public int sendPacket(final ByteBuffer buf)
    {
        ...
        {
            return sendPacketPrivate(buf, buf.position(), buf.limit() - buf.position());
        }
    }
     ...
     * result of a call to pcap_compile(). -1 is returned on failure, in which
     * case pcap_geterr() may be used to display the error text; 0 is returned
     * on success.
     * 
     * @param program the program
     * @return -1 is returned on failure, in which case pcap_geterr() may be
