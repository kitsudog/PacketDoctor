...
public final class PcapUtils
{
     ...
     * @param data opaque, user supplied data object dispatched back to the
     *        handler
     * @return a task object which allows interaction with the underlying
     *         capture loop and thread
     */
        ...
        return new PcapTask<T>(pcap, cnt, data)
        {
            ...
            public void run()
            {
                ...
                while (remaining > 0)
                {
                    ...
                    /*
                     * Yield to other threads on every iteration of the loop,
                     * another words everytime the libpcap buffer has been
                     * completely filled. Except on the first loop, we don't
                     * want to yield but go right into the dispatch loop. Also
                     * having the yield at the top allows the thread to exit
                     * when total count packets have been dispatched and thus
                     * avoid an extra explicit yied, but achive implicit yield
                    ...
                    /*
                     * If not an error, result contains number of packets
                     * dispatched or how many packets fit into the libpcap
                     * buffer
                     */
                    remaining -= result;
                }
     ...
     * @return network interface hardware address or null if unable to retrieve
     *         it
     ...
     */
    private native static <T> int injectLoop(int cnt, int id, PcapPacketHandler<T> handler, T user, PcapPacket packet, JPacket.State state, PcapHeader header,
            JScanner scanner);
