...
public class SearchpathString extends ConfigString
{
     ...
     */
    public SearchpathString(String template, Map<String, String> variables, Properties properties)
    {
    ...
    public SearchPath[] toArray()
    {
        ...
        for (String s : pathContents)
        {
            ...
            if (s.startsWith("File("))
            {
                ...
                list.add(new FilesystemSearch(new ConfigString(content, variables, properties)));

            }
            else if (s.startsWith("Classpath("))
            {
                String content = s.substring("Classpath(".length(), s.length() - 1);
                list.add(new ClasspathSearch(new ConfigString(content, variables, properties)));

            }
            else if (s.startsWith("URL("))
            {
                String content = s.substring("URL(".length(), s.length() - 1);
                list.add(new URLSearch(new ConfigString(content, variables, properties)));
