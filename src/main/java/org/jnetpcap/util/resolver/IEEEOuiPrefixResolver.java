...
public class IEEEOuiPrefixResolver extends AbstractResolver
{
    ...
    /**
     * Default URI path to IEEE raw oui database of manufacturer codes. The URI
     * is {@value #IEEE_OUI_DATABASE_PATH}.
     */
    public final static String IEEE_OUI_DATABASE_PATH = "http://standards.ieee.org/regauth/oui/oui.txt";
    ...
    /**
     * Initializes the resolver by first checking if there are any cached
     * entries, if none, it reads the compressed oui database supplied with
     * jNetPcap in the resource directory.
     */
    @Override
    ...
    /**
     * Download IEEE supplied OUI.txt database of manufacturer prefixes and
     * codes. The file is downloaded using the protocol specified in the URL,
     * parsed and cached indefinately. The machine making the URL connection
     * must have internet connection available as well as neccessary security
     * permissions form JRE in order to make the connection.
     * <p>
     * 
            ...
            while ((s = in.readLine()) != null)
            {
                if (s.contains("(base 16)"))
                {
                    ...
                    if (a.length > 1)
                    {
                        ...
                        /*
                         * Transform some common long terms into short
                         * abbrieviations. Also transform their plural forms and
                         * upper and lower case counterpars.
                         */
                        p = transform(p, a);
    ...
    private String transform(String str, String more, final String singular, final String abbr)
    {
        ...
        /*
         * If after transformation we end up just the short abbreviation, we
         * need to add more to the string if we can from company parts.
         */
        if (str.equals(abbr))
        {
            if (more != null)
    ...
    {
        return ((address[2] < 0) ? address[2] + 256 : address[2]) | ((address[1] < 0) ? address[1] + 256 : address[1]) << 8
