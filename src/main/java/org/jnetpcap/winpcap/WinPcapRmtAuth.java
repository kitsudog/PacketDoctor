...
public final class WinPcapRmtAuth
{
     ...
     * 'type' member of the pcap_rmtauth structure. The 'NULL' authentication
     * has to be equal to 'zero', so that old applications can just put every
     * field of struct pcap_rmtauth to zero, and it does work.
     */
    public final static int RMT_AUTH_NULL = 0;

    /**
     * It defines the username/password authentication. With this type of
     * authentication, the RPCAP protocol will use the username/ password
     * provided to authenticate the user on the remote machine. If the
     * authentication is successful (and the user has the right to open network
     * devices) the RPCAP connection will continue; otherwise it will be
     * dropped. This value has to be used within the 'type' member of the
     * pcap_rmtauth structure.
     */
     ...
     * @param username string containing the username that has to be used on the
     *        remote machine for authentication
     * @param password string containing the password that has to be used on the
     *        remote machine for authentication
     */
