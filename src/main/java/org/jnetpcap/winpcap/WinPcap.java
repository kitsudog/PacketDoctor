 ...
 *  if (pcap == null) {
 *  	LoggerManager.debug.error(errbuf.toString());
 *  	return;
 *  }
 *  pcap.close();	}
 * </pre>
...
public class WinPcap extends Pcap
{
    ...
    private final static ThreadLocal<StringBuffer> buf = new ThreadLocal<StringBuffer>()
    {
     ...
     * remote interface. In that case, it is simply ignored.
     */
    public final static int OPENFLAG_DATATX_UDP = 2;

    /**
     * This flag configures the adapter for maximum responsiveness and can only
     * be used with <code>WinPcap.open</code>. In presence of a large value for
     * nbytes, WinPcap waits for the arrival of several packets before copying
     * the data to the user. This guarantees a low number of system calls, i.e.
     * lower processor usage, i.e. better performance, which is good for
     * applications like sniffers. If the user sets the
     ...
     * suggested for real time applications (like, for example, a bridge) that
     * need the best responsiveness.
     */
    public final static int OPENFLAG_MAX_RESPONSIVENESS = 16;

    /**
     * Defines if the local adapter will capture its own generated traffic and
     * can only be used with <code>WinPcap.open</code>. This flag tells the
     * underlying capture driver to drop the packets that were sent by itself.
     * This is usefult when building applications like bridges, that should
     * ignore the traffic they just sent.
     */
    public final static int OPENFLAG_NOCAPTURE_LOCAL = 8;

    /**
     * Defines if the remote probe will capture its own generated traffic and
     * can only be used with <code>WinPcap.open</code>. In case the remote probe
     ...
     * flag is turned on, the RPCAP traffic is excluded from the capture, so
     * that the trace returned back to the collector is does not include this
     * traffic.
     */
    ...
    /**
     * Flag used with {@link #sendQueueTransmit(WinPcapSendQueue, int)}, to tell
     * kernel to send packets at the rate that is determined by the timestamp
     * with in the sendqueue. The transmittion is synchronized with timestamps.
     */
     ...
     * 'rpcap://1.2.3.4/eth0'). This function is provided in order to help the
     * user creating the source string according to the new format. An unique
     * source string is used in order to make easy for old applications to use
     * the remote facilities. Think about tcpdump, for example, which has only
     ...
     * function to create the source string before passing it to the pcap_open()
     * function.
     * 
     * @param source will contain the complete source string wen the function
     *        returns
     * @param type its value tells the type of the source we want to created
     * @param host an user-allocated buffer that keeps the host (e.g.
     *        "foo.bar.com") we want to connect to. It can be NULL in case we
     *        want to open an interface on a local host
     * @param port an user-allocated buffer that keeps the network port (e.g.
     *        "2002") we want to use for the RPCAP protocol. It can be NULL in
     *        case we want to open an interface on a local host.
     * @param name an user-allocated buffer that keeps the interface name we
     *        want to use (e.g. "eth0"). It can be NULL in case the return
     *        string (i.e. 'source') has to be used with the
     *        pcap_findalldevs_ex(), which does not require the interface name.
     * @param errbuf buffer that will contain the error message (in case there
     *        is one).
     ...
     * @since 1.2
     */
    public static int createSrcStr(Appendable source, int type, String host, String port, String name, Appendable errbuf) throws IOException
    {
     ...
     * @since 1.2
     */
    public native static int createSrcStr(StringBuffer source, int type, String host, String port, String name, StringBuffer errbuf);
     ...
     *         variable.
     */
    public static int createSrcStr(StringBuilder source, int type, String host, String port, String name, StringBuilder errbuf)
    {
     ...
     * present on a remote machine as well. Additionally, it can list all the
     * pcap files available into a given folder. Moreover, pcap_findalldevs_ex()
     * is platform independent, since it relies on the standard
     * pcap_findalldevs() to get addresses on the local machine.
     * </p>
     * <p>
     ...
     * interfaces, and it drops the connection. However, if this function
     * detects that the remote machine is in 'active' mode, the connection is
     * not dropped and the existing socket is used.
     * </p>
     * <p>
     * The 'source' is a parameter that tells the function where the lookup has
     * to be done and it uses the same syntax of the pcap_open().
     * </p>
     * <p>
     * Differently from the pcap_findalldevs(), the interface names (pointed by
     * the alldevs->name and the other ones in the linked list) are already
     * ready to be used in the pcap_open() call. Vice versa, the output that
     * comes from pcap_findalldevs() must be formatted with the new
     * pcap_createsrcstr() before passing the source identifier to the
     * pcap_open().
     * </p>
     * <p>
     ...
     * <p>
     * Warning:<br>
     * There may be network devices that cannot be opened with pcap_open() by
     * the process calling pcap_findalldevs(), because, for example, that
     * process might not have sufficient privileges to open them for capturing;
     * if so, those devices will not appear on the list. The interface list must
     * be deallocated manually by using the pcap_freealldevs().
     * </p>
     * 
     * @param source a char* buffer that keeps the 'source localtion', according
     *        to the new WinPcap syntax. This source will be examined looking
     *        for adapters (local or remote) (e.g. source can be 'rpcap://' for
     ...
     * @since 1.2
     */
    public static int findAllDevsEx(String source, WinPcapRmtAuth auth, List<PcapIf> alldevs, Appendable errbuf) throws IOException
    {
     ...
     * @since 1.2
     */
    public native static int findAllDevsEx(String source, WinPcapRmtAuth auth, List<PcapIf> alldevs, StringBuffer errbuf);
     ...
     *         interface to list
     */
    public static int findAllDevsEx(String source, WinPcapRmtAuth auth, List<PcapIf> alldevs, StringBuilder errbuf)
    {
        final int r = findAllDevsEx(source, auth, alldevs, getBuf());
     ...
     * process does not need to open an adapter; we need just to create the
     * proper filter (by settings parameters like the snapshot length, or the
     * link-layer type) by means of the pcap_compile_nopcap(). The current API
     * of libpcap does not allow to receive a packet and to filter the packet
     * after it has been received. However, this can be useful in case you want
     * to filter packets in the application, instead of into the receiving
     * process. This function allows you to do the job.
     * 
     ...
     */
    public static native int offlineFilter(PcapBpfProgram program, int caplen, int len, ByteBuffer buf);
     ...
     * @since 1.2
     */
    public static native int offlineFilter(PcapBpfProgram program, PcapHeader header, ByteBuffer buffer);
     ...
     * @since 1.2
     */
    public static native int offlineFilter(PcapBpfProgram program, PcapHeader header, JBuffer buffer);
     ...
     */
    @Deprecated
    public static native int offlineFilter(PcapBpfProgram program, PcapPktHdr header, ByteBuffer buf);

    /**
     * Open a generic source in order to capture/send (WinPcap only) traffic.
     * The <code>open</code> replaces all the <code>openXxx()</code> methods
     * with a single call. This method hides the differences between the
     * different <code>openXxx()</code> methods so that the programmer does not
     * have to manage different opening function. In this way, the 'true' open
     * method is decided according to the source type, which is included into
     * the source string (in the form of source prefix). This function can rely
     * on the <code>createSrcStr</code> to create the string that keeps the
     * capture device according to the new syntax, and the
     * <code>parseSrcStr</code> for the other way round.
     * <p>
     ...
     * </ul>
     * 
     * @param source The source name has to include the format prefix according
     *        to the new Source Specification Syntax and it cannot be NULL. <br>
     ...
     *        </ul>
     * @param snaplen length of the packet that has to be retained. For each
     *        packet received by the filter, only the first 'snaplen' bytes are
     *        stored in the buffer and passed to the user application. For
     *        instance, snaplen equal to 100 means that only the first 100 bytes
     *        of each packet are stored.
     * @param flags keeps several flags that can be needed for capturing packet
     * @param timeout read timeout in milliseconds. The read timeout is used to
     *        arrange that the read not necessarily return immediately when a
     *        packet is seen, but that it waits for some amount of time to allow
     *        more packets to arrive and to read multiple packets from the OS
     *        kernel in one operation. Not all platforms support a read timeout;
     *        on platforms that don't, the read timeout is ignored.
     * @param auth a pointer to a 'struct pcap_rmtauth' that keeps the
     *        information required to authenticate the user on a remote machine.
     *        In case this is not a remote capture, this pointer can be set to
     *        NULL.
     * @param errbuf a pointer to a user-allocated buffer which will contain the
     *        error in case this function fails. The pcap_open() and
     *        findalldevs() are the only two functions which have this
     *        parameter, since they do not have (yet) a pointer to a pcap_t
     *        structure, which reserves space for the error string. Since these
     *        functions do not have (yet) a pcap_t pointer (the pcap_t pointer
     *        is NULL in case of errors), they need an explicit 'errbuf'
     *        variable. 'errbuf' may also be set to warning text when
     *        pcap_open_live() succeds; to detect this case the caller should
     *        store a zero-length string in 'errbuf' before calling
     *        pcap_open_live() and display the warning to the user if 'errbuf'
     ...
     * @since 1.2
     */
    public static WinPcap open(String source, int snaplen, int flags, int timeout, WinPcapRmtAuth auth, Appendable errbuf) throws IOException
    {
     ...
     * @since 1.2
     */
    public native static WinPcap open(String source, int snaplen, int flags, int timeout, WinPcapRmtAuth auth, StringBuffer errbuf);
     ...
     *         keeps the error message.
     */
    public static WinPcap open(String source, int snaplen, int flags, int timeout, WinPcapRmtAuth auth, StringBuilder errbuf)
    {
        final WinPcap r = open(source, snaplen, flags, timeout, auth, getBuf());
     ...
     * @param snaplen filters generated using the pcap structure will truncate
     *        captured packets to this length
     * @return WinPcap structure that can only be used to generate filter code
     *         and none of its other capture methods should be called or null if
     *         error occured
     ...
     * </p>
     * 
     * @param device buffer containing a C, '\0' terminated string with the the
     *        name of the device
     * @param snaplen amount of data to capture per packet; (see special note in
     *        doc comments about when this argument is ignored even when
     *        non-zero)
     * @param promisc 1 means open in promiscious mode, a 0 means
     *        non-propmiscous
     * @param timeout timeout in ms
     * @param errbuf a buffer that will contain any error messages if the call
     *        to open failed
     ...
     * @since 1.2
     */
    public static WinPcap openLive(String device, int snaplen, int promisc, int timeout, Appendable errbuf) throws IOException
    {
     ...
     * @since 1.2
     */
    public native static WinPcap openLive(String device, int snaplen, int promisc, int timeout, StringBuffer errbuf);
     ...
     * @see Pcap#openLive(String, int, int, int, StringBuilder)
     */
    public static WinPcap openLive(String device, int snaplen, int promisc, int timeout, StringBuilder errbuf)
    {
        final WinPcap r = openLive(device, snaplen, promisc, timeout, getBuf());
     ...
     * @since 1.2
     */
    public static WinPcap openOffline(String fname, Appendable errbuf) throws IOException
    {
     ...
     * @throws IOException any IO errors produced by the appendable
     */
    private static void toAppendable(StringBuffer buf, Appendable appendable) throws IOException
    {
    ...
    /**
     * dumps the network traffic from an interface to a file. Using this
     * function the dump is performed at kernel level, therefore it is more
     * efficient than using Pcap.dump(). The parameters of this function are an
     * interface descriptor (obtained with openLive()), a string with the name
     * of the dump file, the maximum size of the file (in bytes) and the maximum
     * number of packets that the file will contain. Setting maxsize or maxpacks
     * to 0 means no limit. When maxsize or maxpacks are reached, the dump ends.
     * liveDump() is non-blocking, threfore Return immediately. liveDumpEnded()
     * can be used to check the status of the dump process or to wait until it
     * is finished. Pcap.close() can instead be used to end the dump process.
     * Note that when one of the two limits is reached, the dump is stopped, but
     * the file remains opened. In order to correctly flush the data and put the
     * file in a consistent state, the adapter must be closed with Pcap.close().
     * 
     ...
     * pcap_live_dump_ended() informs the user about the limits that were set
     * with a previous call to pcap_live_dump() on the interface pointed by p:
     * if the return value is nonzero, one of the limits has been reched and the
     * dump process is currently stopped. If sync is nonzero, the function
     * blocks until the dump is finished, otherwise Return immediately. Warning:
     * if the dump process has no limits (i.e. if the maxsize and maxpacks
     * arguments of pcap_live_dump() were both 0), the dump process will never
     * stop, therefore setting sync to TRUE will block the application on this
     * call forever.
     * 
     ...
     * content of a queue to the wire. p is a pointer to the adapter on which
     * the packets will be sent, queue points to a pcap_send_queue structure
     ...
     * parameter, an error occurred during the send. The error can be caused by
     * a driver/adapter problem or by an inconsistent/bogus send queue. Note:
     * Using this function is more efficient than issuing a series of
     * pcap_sendpacket(), because the packets are buffered in the kernel driver,
     * so the number of context switches is reduced. Therefore, expect a better
     * throughput when using pcap_sendqueue_transmit. When Sync is set to TRUE,
     * the packets are synchronized in the kernel with a high precision
     * timestamp. This requires a non-negligible amount of CPU, but allows
     * normally to send the packets with a precision of some microseconds
     * (depending on the accuracy of the performance counter of the machine).
     * Such a precision cannot be reached sending the packets with
     * pcap_sendpacket().
     * 
     * @param queue queue containing the data to be sent
     * @param synch if it is non-zero, the packets are sent respecting the
     *        timestamps, otherwise they are sent as fast as possible
     * @return amount of bytes actually sent; error if less then queues len
     *         parameter
     */
    public native int sendQueueTransmit(final WinPcapSendQueue queue, final int synch);
     ...
     * pcap_setmintocopy() changes the minimum amount of data in the kernel
     * buffer that causes a read from the application to return (unless the
     * timeout expires). If the value of size is large, the kernel is forced to
     * wait the arrival of several packets before copying the data to the user.
     * This guarantees a low number of system calls, i.e. low processor usage,
     * and is a good setting for applications like packet-sniffers and protocol
     * analyzers. Vice versa, in presence of a small value for this variable,
     * the kernel will copy the packets as soon as the application is ready to
     * receive them. This is useful for real time applications that need the
     * best responsiveness from the kernel.
     * 
    ...
    /**
     * Set the working mode of the interface p to mode. Valid values for mode
     * are MODE_CAPT (default capture mode) and MODE_STAT (statistical mode).
     * 
    ...
    /**
     * <p>
     * Define a sampling method for packet capture. This function allows
     * applying a sampling method to the packet capture process. The currently
     * sampling methods (and the way to set them) are described into the struct
     * pcap_samp. In other words, the user must set the appropriate parameters
     ...
     * parameters must be applied before starting the capture. If they are
     * applied when the capture is in progress, the new settings are ignored.
     * Sampling works only when capturing data on Win32 or reading from a file.
     * It has not been implemented on other platforms. Sampling works on remote
     * machines provided that the probe (i.e. the capturing device) is a Win32
     * workstation.
     * </p>
     * 
