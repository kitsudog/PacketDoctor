 ...
 * if (r != queue.getLen())
 * {
 *     LoggerManager.debug.error(pcap.getErr());
 *     return;
 * }
 * 
 * </pre>
...
public class WinPcapSendQueue extends JStruct
{
     ...
     * an exception thrown. The properties of the buffer are used as follows.
     * The start of the buffer is always with index 0, and end of queue content
     * at current buffer's limit property (comparible to pcap_send_queue.len).
     * The capacity property (comparible to pcap_send_queue.maxlen) determines
     * maximum amount of data that can be further stored in the buffer.
     * </p>
     * <p>
     * Note that changing properties of the buffer after creating this queue
     * object, will have immediate effect up on the queue. You do not have to
     * use the queue's provided methods to change the limit property. This
     * should allow of external addition of the
     * </p>
     * 
     ...
     * PcapPktHdr structure with the timestamp and the length of the packet,
     * data points to a buffer with the data of the packet. The PcapPktHdr
     * structure is the same used by WinPcap and libpcap to store the packets in
     * a file, therefore sending a capture file is straightforward. 'Raw packet'
     * means that the sending application will have to include the protocol
     * headers, since every packet is sent to the network 'as is'. The CRC of
     * the packets needs not to be calculated, because it will be transparently
     * added by the network interface.
     * 
     * @param header all fields need to be initialized as they are all used
     * @param data Buffer containing packet data. The buffer's position and
     *        limit properties determine the area of the buffer to be copied
     *        into the queue. The length of the data must much what is in the
     *        header. Also the queue has to be large enough to hold all of the
     *        data, or an exception will be thrown.
     ...
     * @param data Buffer containing packet data. The length of the data must
     *        much what is in the header. Also the queue has to be large enough
     *        to hold all of the data, or an exception will be thrown.
